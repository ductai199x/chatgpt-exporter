...
(function (JSZip, html2canvas) {
  ...
  async function getCompressedBase64FromImageUrl(url, quality = 0.95) {
    const response = await fetch(url);
    const blob = await response.blob();
    return await blobToCompressedBase64(blob, quality);
  }
  function blobToCompressedBase64(blob, quality) {
    return new Promise((resolve, reject) => {
      const img = new Image();
  ...
  function formatFileId(fileIdWithoutHyphens) {
    return fileIdWithoutHyphens.replace(
      /^(.{8})(.{4})(.{4})(.{4})(.{12})$/,
      "$1-$2-$3-$4-$5"
    );
  }
  async function scrapeRenderedImage(assetPointer) {
  ...
  async function replaceImageAssets(conversation) {
    const isMultiModalInputImage = (part) => {
      return typeof part === "object" && part !== null && "content_type" in part && part.content_type === "image_asset_pointer" && "asset_pointer" in part && typeof part.asset_pointer === "string" && (part.asset_pointer.startsWith("file-service://") || part.asset_pointer.startsWith("sediment://"));
    };
    ...
    await Promise.all([
      ...imageAssets.map(async (asset) => {
        try {
          ...
          console.error("Failed to scrape image asset", error2);
        }
      }),
  ...
  var Portal$2 = D((props, forwardedRef) => {
    ...
    return container ? xn.createPortal(/* @__PURE__ */ u$5(Primitive.div, { ...portalProps, ref: forwardedRef }), container) : null;
  });
